...
# คู่มือ Format การสรุปโน้ตวิชา Natural Language Processing (1323404)
...
| --- | --- | --- | --- |
| `tags` | `[nlp, weekN, <topic-tag>]` | `[nlp, weekN, moc]` | topic-tag เลือก 1-2 คำที่สื่อเนื้อหา |
| `course` | `1323404` | `1323404` | ตายตัว ไม่ต้องเปลี่ยน |
...
| :--- | :--- | :--- | :--- |
| **Activity Diagram** | `flowchart TD` หรือ `LR` | ขั้นตอนการประมวลผลข้อความ (Pipeline), Tokenization, Encoding-Decoding | เริ่มต้น `((●))`, กิจกรรม `([Action])`, เงื่อนไข `{Decision?}`, สิ้นสุด `(((●)))` |
| **Sequence Diagram** | `sequenceDiagram` | ลำดับการเรียกใช้ Prompt, RAG Retrieval Pipeline, Agent-Tool Calling | `actor`, `participant`, `->>`, `-->>`, `alt/else`, `loop` |
| **State Machine Diagram** | `stateDiagram-v2` | สถานะโมเดล, Beam Search decoding states, Lifecycle การเทรน | `[*]`, สถานะ `State`, ทรานซิชัน `-->`, เงื่อนไข `[condition]` |
| **Class / Component Diagram** | `classDiagram` หรือ `flowchart` | โครงสร้างสถาปัตยกรรมโมเดล (Transformer Layers, Multi-Head Attention) | `class`, `subgraph` |

> [!important] ข้อปฏิบัติในการเขียน Diagram
> - **Bilingual Text:** ข้อความอธิบายเป็นภาษาไทย พร้อมระบุศัพท์เทคนิคภาษาอังกฤษกำกับ (เช่น `([สกัดเวกเตอร์คำ <br> (Extract Word Embeddings)])`)
